...
void main() {
  ...
  print(
    "A rendelt termékek darabszáma a 21. napon PL: ${elso} TV: ${masodik} NR: ${harmadik} ",
  );
  print("8.feladat");
  ...
  for (var i = 0; i < 30; i++) {
    int index = i ~/ 10;
    PLszamok[index] += osszes("PL", i + 1, rendelesek, 1);
    TVszamok[index] += osszes("TV", i + 1, rendelesek, 1);
    NRszamok[index] += osszes("NR", i + 1, rendelesek, 1);
  }
...
int osszes(
  String varos,
  int nap,
  List<Map<String, dynamic>> rendelsek, [
  int verzio = 0,
]) {
  int eredmeny = 0;
